...
test_container() {
    TAG=$1
    docker run --detach --env TRAVIS --env TRAVIS_BRANCH --env DISPLAY --env RESOLUTION="1024x768" --volume /tmp/.X11-unix:/tmp/.X11-unix test-xfce-ubuntu:$TAG /usr/bin/dbus-run-session /usr/bin/xfce4-session > .docker_ID
