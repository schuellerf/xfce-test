...
docker exec --detach $(cat .docker_ID) /usr/bin/dbus-daemon --system || docker logs $(cat .docker_ID)
docker exec --detach $(cat .docker_ID) /usr/bin/dbus-run-session /usr/local/bin/ldtp || docker logs $(cat .docker_ID)
docker exec --detach $(cat .docker_ID) /container_scripts/start_pulseaudio.sh
